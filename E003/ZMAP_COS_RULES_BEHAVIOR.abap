...
*&---------------------------------------------------------------------*
managed implementation in class zbp_zmap_cos_rules_cds unique;
strict ( 2 );

define behavior for ZMAP_COS_RULES_CDS alias COSRules
persistent table zmap_cos_rules
lock master
...
*&---------------------------------------------------------------------*
*& Behavior Implementation: ZBP_ZMAP_COS_RULES_CDS
*& Description: Behavior Implementation for COS Mapping
*&---------------------------------------------------------------------*
CLASS zbp_zmap_cos_rules_cds DEFINITION
  PUBLIC
  ABSTRACT
  FINAL
  FOR BEHAVIOR OF ZMAP_COS_RULES_CDS.
...
CLASS zbp_zmap_cos_rules_cds IMPLEMENTATION.

  METHOD validateData.
    " Validate required fields
    LOOP AT keys INTO DATA(key).
      READ ENTITIES OF ZMAP_COS_RULES_CDS IN LOCAL MODE
        ENTITY COSMapping
  ...
  METHOD activate.
    ...
    LOOP AT keys INTO DATA(key).
      ...
      LOOP AT lt_cos_mapping INTO DATA(ls_cos_mapping).
        MODIFY ENTITIES OF ZMAP_COS_RULES_CDS IN LOCAL MODE
          ENTITY COSMapping
